...
export async function POST(req: Request) {
  ...
  if (file.size > MAX_SIZE_BYTES) return NextResponse.json({ error: "Slika je prevelika (max 5MB)" }, { status: 400 });
  ...
  // Vercel Blob (produkcija) ili lokalni filesystem (razvoj)
  if (process.env.BLOB_READ_WRITE_TOKEN) {
    try {
      ...
      return NextResponse.json({ error: "Upload nije uspio. Pokušajte ponovo." }, { status: 500 });
    }
  }
  ...
  try {
    ...
    return NextResponse.json({ error: "Upload nije uspio — filesystem nije dostupan. Kontaktirajte admina." }, { status: 500 });
  }
}
